...
const makeDb = () => {
  ...
  const db = drizzle({ client, schema: { budgets } });
  client.exec(`
...
describe("budgetRepository", () => {
  ...
  beforeEach(() => {
    repo = budgetRepositoryFactory({ database: makeDb() });
  });
  ...
  describe("findAll", () => {
    ...
    describe("given multiple budgets have been created", () => {
      ...
      beforeEach(async () => {
        a = await repo.create({ categoryId: "cat-1", limitInEuro: 300 }, "user-1");
        b = await repo.create({ categoryId: "cat-2", limitInEuro: 1000 }, "user-1");
      });
  ...
  describe("create", () => {
    describe("given a valid input", () => {
      it("should return the budget with all input fields", async () => {
        const result = await repo.create({ categoryId: "cat-1", limitInEuro: 300 }, "user-1");
      ...
      it("should persist the budget so findAll can retrieve it", async () => {
        const created = await repo.create({ categoryId: "cat-1", limitInEuro: 300 }, "user-1");
    ...
    describe("given two separate create calls", () => {
      it("should assign a unique id to each", async () => {
        const a = await repo.create({ categoryId: "cat-1", limitInEuro: 300 }, "user-1");
        const b = await repo.create({ categoryId: "cat-2", limitInEuro: 1000 }, "user-1");
